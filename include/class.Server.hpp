...
class Server
{
  ...
  private:
    ...
    bool hasUnallowedChar(t_str_c &strToCheck, t_str_c &unallowedChars) const;

    /* user/channel handling */
    bool isErasable(int socket) const;
    void eraseUserFromAllChannels(int socket);
    void createChannelBy(int socket, t_str_c &channelName, t_str_c &channelKey);
    void addUserToChannels(int socket, t_vec_str_c &channelNames, t_vec_str_c &channelKeys);
    void broadcast(t_str_c &sender, t_str_c &channelName, t_str_c &nicknameKicked, t_str_c &message, t_str_c &command);
    ...
    void RPL_PART(int socketSender, int socketTarget, t_str_c &channelName, t_str_c &message);
    void RPL_KICK(int socketSender, int socketTarget, t_str_c &channelName, t_str_c &nicknameKicked, t_str_c &message);
    void RPL_IFTOPIC(int socket, t_str_c &channelName, t_str_c &topic);
    void RPL_PRIVMSG(int socket, t_str_c &target, t_str_c &message);
    void RPL_PRIVMSG_CHANNEL(int socketSender, int socketTarget, t_str_c &channelName, t_str_c &message);
    void RPL_INVITING(int socketSender, int socketTarget, t_str_c &channelName, t_str_c &target);
    ...
    void ERR_USERNOTINCHANNEL(int socketSender, int socketTarget, t_str_c &channelName);
